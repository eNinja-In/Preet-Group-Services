...
export const fetchData = async (engineNo, setData, setNotification, setPopUp) => {
    try {
        const response = await fetch(
            `${import.meta.env.VITE_SERVER_LINK}/api/auth/login-user`,
            {
                method: "POST",
                body: JSON.stringify({ engineNo }),
        ...
        if (data.success) {
            setData(data.data);
            setNotification({ message: 'Data fetched successfully!', type: 'success' });
        } else {
