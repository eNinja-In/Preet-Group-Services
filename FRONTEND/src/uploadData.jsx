...
import { registerCombine, updateCombine } from "./components/helper/combineHelper";
...
export function BulkUpdate() {
  ...
  // --- Processing Logic for Updating Data ---
  const processExcelData = async (data) => {
    stopFlag.current = false;
    setIsProcessing(true);
    ...
    for (let i = 0; i < data.length; i++) {
      ...
      // Convert Excel → Backend update payload
      const payload = {
        combineModel: row["MODEL"] || "NaN",
        engineNo: row["ENGINE NO"] || "NaN",
        chassisNo: row["CHASSIS NO"] || "NaN",
      ...
      // >>> CRITICAL LOGGING STEP: Check what is being sent
      console.log(`[Row ${i + 1}] Sending Update Payload:`, payload);

      let res = await updateCombine(payload);

      // Retry logic (up to 2 attempts total)
      let attempt = 1;
      while (!res.success && attempt < 2) {
        res = await updateCombine(payload);
        attempt++;
      }
  ...
  return (
    <div className="p-6">
      <h1 className="text-2xl font-bold mb-3">📄 Bulk Excel Update</h1>
